...
通过执行 max += max + 1，即 max += 4，更新后的 max 将变为 7。这意味着通过添加 4 到数组中，我们现在可以覆盖从 1 到 7 的所有数字。
这种策略是基于这样一个事实：每添加一个数字，我们都希望最大限度地扩展覆盖范围。通过添加 max + 1，我们保证了覆盖范围以最大可能的方式增加，因为这个新添加的数字与之前可覆盖的所有数字组合都可能产生新的和，从而扩大覆盖范围。
 * 
 **/
